...
async function initClaude(cwd: string): Promise<void> {
  console.log("Running claude /init...");
  await execAsync("claude /init --dangerously-skip-permissions", {
    cwd,
    timeout: 120000,
    env: { ...process.env, PATH: process.env.PATH },
    maxBuffer: 10 * 1024 * 1024,
  });
  console.log("Claude /init completed");
}

async function commitAndPush(cwd: string, message: string): Promise<boolean> {
  // Exclude CLAUDE.md from commit (generated by /init)
